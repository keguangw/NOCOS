...
def read_ice_data(configs):
    ...
    if '?' in configs['ice_filename'] or '*' in configs['ice_filename']:
       icedataraw = xr.open_mfdataset(configs['ice_filename'], 
                                      concat_dim=configs['coordinates']['time_name'])
    else:
       icedataraw = xr.open_dataset(configs['ice_filename']) 
